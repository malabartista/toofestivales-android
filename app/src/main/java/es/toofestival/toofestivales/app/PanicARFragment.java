...
public class PanicARFragment extends PARFragment {
     ...
     */
    public void loadARPosts() {

        int offset = 1;
        String url = Config.POSTS_URL;
        url += "?scope=future";
        StringRequest request = new StringRequest(Request.Method.GET, url + "&page=" + offset + "&per_page=50", new Response.Listener<String>() {
            @Override
            public void onResponse(String s) {
                Gson gson = new Gson();
                List<Object> listPosts = (List) gson.fromJson(s, List.class);
                if(listPosts!=null) {
                    for (int i = 0; i < listPosts.size(); ++i) {
                        ...
                        if (!p.getLocation_country().isEmpty() && !p.getLocation_town().isEmpty()) {
                            description = p.getLocation_town() + ", " + p.getLocation_country();
                        }
                        PARController.getInstance().addPoi(createPoi(title, description, lat, lng));
                    }
                } else {
                }
            }
            ...
            public void onErrorResponse(VolleyError volleyError) {
                Toast.makeText(AppController.getInstance(), R.string.error_load_posts, Toast.LENGTH_LONG).show();
            }
        });
        RequestQueue rQueue = Volley.newRequestQueue(AppController.getInstance());
        rQueue.add(request);
    }
